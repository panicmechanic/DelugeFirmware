...
gotError:
    ...
    error = arrivedInNewFolder(0, searchFilename.get(), "SONGS");		if (error) goto gotError;
    	...
    	if (audioFile->numReasonsToBeLoaded) {
    		...
    		if (audioFile->type == AUDIO_FILE_TYPE_SAMPLE) {
				...
				if (!((Sample*)audioFile)->tempFilePathForRecording.isEmpty()) {
					FRESULT result = f_rename(((Sample*)audioFile)->tempFilePathForRecording.get(), audioFile->filePath.get());
					if (result == FR_OK) {
    		...
    		if (collectingSamples || !audioFile->loadedFromAlternatePath.isEmpty()) {
				...
				if (!audioFile->loadedFromAlternatePath.isEmpty()) {
					...
					sourceFilePath = (audioFile->type != AUDIO_FILE_TYPE_SAMPLE || ((Sample*)audioFile)->tempFilePathForRecording.isEmpty()) ?
							audioFile->filePath.get() :
							((Sample*)audioFile)->tempFilePathForRecording.get(); // It may still have a temp path if for some reason we failed to move it, above
				}
				...
				if (collectingSamples) {
					...
					if (!memcasecmp(normalFilePath, "SAMPLES/RECORD/REC", 18)
						...
						if (audioFile->filePath.getLength() - fileNamePos == 12 && !strcasecmp(normalFilePath + fileNamePos + 8, ".WAV")) {
							...
							needToPretendLoadedAlternate = true; // We don't have that alternate path yet, so just make a note to do it below.
						}
					}
...
failAfterOpeningSourceFile:
					...
					// it doesn't clash with anything.
						...
						error = newSongAlternatePath.concatenateAtPos(fileName, dirPathLengthNew);			if (error) goto failAfterOpeningSourceFile;
					}
				...
				if (error == ERROR_FILE_ALREADY_EXISTS) {} // No problem - the audio file was already there from before, so we don't need to copy it again now.
...
fail3:
						...
						UINT bytesWritten;
						result = f_write(&recorderFileSystemStuff.currentFile, storageManager.fileClusterBuffer, bytesRead, &bytesWritten);
    ...
    if (fileAlreadyExisted) {
    	...
    	while (true) {
    		error = filePathDuringWrite.set("SONGS/TEMP");					if (error) goto gotError;
    		error = filePathDuringWrite.concatenateInt(tempFileNumber, 4);	if (error) goto gotError;
    		error = filePathDuringWrite.concatenate(".XML");				if (error) goto gotError;
    ...
    error = storageManager.closeFileAfterWriting(filePathDuringWrite.get(), "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<song\n", "\n</song>\n");
    if (error) goto gotError;
