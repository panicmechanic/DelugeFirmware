...
char const* StorageManager::readNextTagOrAttributeName() {
    ...
    default:
    	...
    	numericDriver.freezeWithError("E365"); // Can happen with invalid files, though I'm implementing error checks whenever a user alerts me to a scenario. Fraser got this, Nov 2021.
#else
...
void StorageManager::skipUntilChar(char endChar) {
	...
	do {
		while (fileBufferCurrentPos < currentReadBufferEndPos
				&& fileClusterBuffer[fileBufferCurrentPos] != endChar) {
			fileBufferCurrentPos++;
		}
...
int StorageManager::readStringUntilChar(String* string, char endChar) {
	...
	do {
		...
		while (bufferPosNow < currentReadBufferEndPos
				&& fileClusterBuffer[bufferPosNow] != endChar) {
			bufferPosNow++;
		}
...
void StorageManager::exitTag(char const* exitTagName) {
	...
	while (tagDepthFile >= tagDepthCaller) {
		...
		default:
			if (ALPHA_OR_BETA_VERSION) numericDriver.freezeWithError("AAAA"); // Really shouldn't be possible anymore, I feel fairly certain...
			__builtin_unreachable();
		}
	}
...
int StorageManager::checkSpaceOnCard() {
	...
	return fileSystemStuff.fileSystem.free_clst ? NO_ERROR : ERROR_SD_CARD_FULL; // This doesn't seem to always be 100% accurate...
}
...
cutFolderPathAndTryCreating:
    		...
    		else if (result == FR_NO_PATH) { // If that folder couldn't be created because its parent folder didn't exist...
    	...
    	// Otherwise, just return the appropriate error.
    		...
    		if (error == ERROR_SD_CARD) error = ERROR_WRITE_FAIL; // Get a bit more specific if we only got the most general error.
    		return error;
    	}
    }
...
int StorageManager::closeFileAfterWriting(char const* path, char const* beginningString, char const* endString) {
	if (fileAccessFailedDuring) return ERROR_WRITE_FAIL; // Calling f_close if this is false might be dangerous - if access has failed, we don't want it to flush any data to the card or anything
	int error = writeBufferToFile();
...
int StorageManager::openXMLFile(FilePointer* filePointer, char const* firstTagName, char const* altTagName, bool ignoreIncorrectFirmware) {
...
int StorageManager::tryReadingFirmwareTagFromFile(char const* tagName, bool ignoreIncorrectFirmware) {
	...
	else if (!strcmp(tagName, "earliestCompatibleFirmware")) { // If this tag doesn't exist, it's from old firmware so is ok
		char const* firmwareVersionString = readTagOrAttributeValue();
...
bool StorageManager::closeFile() {
	if (fileAccessFailedDuring) return false; // Calling f_close if this is false might be dangerous - if access has failed, we don't want it to flush any data to the card or anything
	FRESULT result = f_close(&fileSystemStuff.currentFile);
	return (result == FR_OK);
}
...
void StorageManager::writeFirmwareVersion() {
	writeAttribute("firmwareVersion", "4.1.4");
}
...
// clip may be NULL
int StorageManager::loadInstrumentFromFile(Song* song, InstrumentClip* clip, int instrumentType, bool mayReadSamplesFromFiles, Instrument** getInstrument, FilePointer* filePointer, String* name, String* dirPath) {
...
deleteInstrumentAndGetOut:
	...
	if (!song->getBackedUpParamManagerPreferablyWithClip((ModControllableAudio*)newInstrument->toModControllable(), NULL)) {
	...
	if (newInstrument->type == INSTRUMENT_TYPE_KIT) {
		...
		for (Drum* thisDrum = kit->firstDrum; thisDrum; thisDrum = thisDrum->next) {
			if (thisDrum->type == DRUM_TYPE_SOUND) {
				...
				if (!currentSong->getBackedUpParamManagerPreferablyWithClip(soundDrum, NULL)) { // If no backedUpParamManager...
...
int StorageManager::readMIDIParamFromFile(int32_t readAutomationUpToPos, MIDIParamCollection* midiParamCollection, int8_t* getCC) {
    ...
    while (*(tagName = readNextTagOrAttributeName())) {
        if (!strcmp(tagName, "cc")) {
        	...
        	else if (!strcasecmp(contents, "none") || !strcmp(contents, "120")) // We used to write 120 for "none", pre V2.0
        		cc = CC_NUMBER_NONE;
        	else
